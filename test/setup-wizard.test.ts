...
test("the external packages are one question, and declining installs none of them", async () => {
  ...
  assert.deepEqual(acceptedManifest?.modules, { lsp: true, context: true, mcp: true, subagents: true, goal: true, memory: true });

  const declined = scriptedWizard(["", "", "", "", "no", "yes"]);
  const declinedManifest = await runSetupWizard(declined.io);
  assert.deepEqual(declinedManifest?.modules, { lsp: false, context: false, mcp: false, subagents: false, goal: false, memory: false });
